function [objects, train_id] = readLabels(label_dir,img_idx)

% parse input file
file_list = dir(label_dir);
fid = fopen(sprintf('%s/%s',label_dir,file_list(3+img_idx).name),'r');
train_id = file_list(3+img_idx).name;
