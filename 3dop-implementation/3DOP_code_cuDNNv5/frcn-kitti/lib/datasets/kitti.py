...
class kitti(datasets.imdb):
    ...
    def depth_image_path_from_index(self, index):
        ...
        Construct an image path from the image's "index" identifier.
        """
        depth_image_path = os.path.join(self._data_path, 'disparity',
               index + self._image_ext)
    ...
    def _load_proposals(self, method, gt_roidb):
        ...
        box3D_list = []
        top_k = self.config['top_k']
        # '3DOP': class-dependent proposals
        # '3DOP-generic': class-independent proposals
        valid_methods = ['MCG', 'selective_search', 'edge_boxes_70', '3DOP', '3DOP-generic']
        ...
        for i, index in enumerate(self._image_index):
            ...
            if method in ['3DOP']:
                ...
                boxes3D = np.zeros((0,7), dtype=np.float32)
                for ci, c in enumerate(self._classes):
                    ...
                    raw_boxes3D = sio.loadmat(box_file)['boxes3D'].astype(np.float32)
                    this_boxes3D = (raw_boxes3D[:top_k, 0:7])
                    boxes3D = np.vstack((boxes3D, this_boxes3D))
            else:
                box_file = osp.join(self.cache_path, '..', 'proposals',
                                method, 'mat',
                                self._get_box_file(index))
                ...
                raw_boxes3D = sio.loadmat(box_file)['boxes3D']
                boxes3D = (raw_boxes3D[:top_k, :] - 1).astype(np.float32)
        ...
        return self.create_roidb_from_box_list(box_list, box3D_list, gt_roidb)
    ...
    def _load_kitti_annotation(self, index):
        ...
        for line in lines:
            ...
            boxes3D[ix, :] = [ry, l, h, w, tx, ty, tz]
            gt_classes[ix] = cls
            overlaps[ix, cls] = 1.0

        alphas.resize(ix+1)
        boxes.resize(ix+1, 4)
        boxes3D.resize(ix+1, 7)
