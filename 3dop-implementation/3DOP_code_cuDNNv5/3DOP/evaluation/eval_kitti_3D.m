...
fh = figure;
eval_recall_dist(db, {methods.best_candidates3D_file}, methods, 2000, ...
    0.25, fh, true, 'SouthWest', save_prefix);

% 3D Recall-Overlap curves using 2000 proposals
fh = figure;
eval_recall_overlap_3D({methods.best_candidates3D_file}, methods, 2000, ...
    fh, true, 'SouthWest', save_prefix);

% 3D Recall-Proposal curves
eval_recall_3D({methods.best_candidates3D_file}, methods, save_prefix);
...
%%
function results = eval_recall_3D(iou_files, methods, save_prefix)
