...
iou_thrs = [0.5, 0.7];
for i = 1 : length(iou_thrs)
    fh = figure;
    eval_recall_dist(db, {methods.best_candidates_file}, methods, ...
        2000, iou_thrs(i), fh, true, 'SouthWest', save_prefix);
end

% Recall-Overlap curves using 500, 1000, 2000 proposals
names_in_plot = true;
num_boxes = [500, 1000, 2000];
for i = 1 : length(num_boxes)    
    fh = figure;
    eval_recall_overlap({methods.best_candidates_file}, methods, ...
        num_boxes(i), fh, names_in_plot, 'NorthEast', save_prefix);
end
