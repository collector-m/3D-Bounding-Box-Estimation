...
vector<disp> sparseDisparityGrid (float* D,const int *dims,int32_t* roi,int roim, int32_t step_size) {
  ...
  for (int m = 0; m < roim; m++) {
    int32_t u = roi[m];
    int32_t v = roi[m + roim];
    float d = *(D+getAddressOffsetImage(u,v,width));
    if (d>=1)
      d_list.push_back(disp(u,v,d));
  }
...
void computeRoadPlaneEstimate (float* D,const int *dims,float* plane,int32_t* roi,int roim,int32_t num_samples,float d_threshold) {
  
  // random seed
  srand( time(NULL));
  
  // get list with disparities
  vector<disp> d_list = sparseDisparityGrid(D,dims,roi,roim,5);
...
void mexFunction (int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) {
    ...
    if(!mxIsInt32(prhs[1]) || mxGetNumberOfDimensions(prhs[1])!=2 || mxGetN(prhs[1])!=2)
      mexErrMsgTxt("Input roi must be a int32 nx2 matrix.");
    ...
    // do computation
    computeRoadPlaneEstimate(D,dims,plane,roi,roim,num_samples,d_threshold);
}
